...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cmu418_mask maskAll, maskCount, maskClamp;
  ...
  for (int i=0; i<N;) {
    ...
    _cmu418_vgt_int(maskCount, count, zero, maskAll);        //   bool tmp = count > 0;
    while(_cmu418_cntbits(maskCount)) {                      //   while(tmp) {
      ...
      _cmu418_vgt_int(maskCount, count, zero, maskAll);      //      tmp = count > 0;
    }                                                        //   }
