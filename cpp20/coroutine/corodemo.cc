/*
Demo from: https://www.scs.stanford.edu/~dm/blog/c++-coroutines.html
...
struct Awaiter {
  ...
  // constexpr removed to enable printing
  bool await_ready() const noexcept {
    std::cout << "[Awaiter] In await_ready function\n";
    return false;
  }
  void await_suspend(std::coroutine_handle<> h) {
    std::cout << "[Awaiter] In await_suspend function\n";
    *hp_ = h;
  }
  // constexpr removed to enable printing
...
counter(std::coroutine_handle<> *continuation_out)
{
  ...
  for (unsigned i = 0;; ++i) {
    std::cout << "[coroutine] before co_await." << std::endl;
    co_await a;
    std::cout << "[coroutine] after co_wait. counter: " << i << std::endl;
  }
}
...
main1()
{
  ...
  std::cout << "[main1] before call coroutine func." << std::endl;
  counter(&h);
  std::cout << "[main1] after call coroutine func and fetch handle." << std::endl;
  for (int i = 0; i < 3; ++i) {
    std::cout << "[main1] before call coroutine_handle<>()." << std::endl;
    h();
    std::cout << "[main1] after call coroutine_handle<>()." << std::endl;
  }
  h.destroy();
}
...
struct ReturnObject2 {
  ...
  operator std::coroutine_handle<promise_type>() const {
    std::cout << "[ReturnObject] operator std::coroutine_handle<promise_type>()." << std::endl;
    return h_;
  }
  // A coroutine_handle<promise_type> converts to coroutine_handle<>
  operator std::coroutine_handle<>() const {
    std::cout << "[ReturnObject] operator std::coroutine_handle<>()." << std::endl;
    return h_;
  }
};
...
main2()
{
  std::cout << "[main2] before call coroutine func." << std::endl;
  std::coroutine_handle<> h = counter2();
  std::cout << "[main2] after call coroutine func and return handle." << std::endl;
  for (int i = 0; i < 3; ++i) {
